...
const navItems = [
  ...
  {
    name: 'NPM',
    href: 'https://www.npmjs.com/package/feedget',
    external: true,
  },
]
    ...
    <Popover>
      <div className='relative pt-6 px-4 sm:px-6 lg:px-8'>
        ...
        >
          <div className='flex items-center flex-grow flex-shrink-0 lg:flex-grow-0'>
            <div className='flex items-center justify-between w-full pr-3 md:pr-0'>
              ...
              {location.pathname === '/dashboard' && (
                <ThemeToggler className='ml-6 !text-zinc-900 dark:text-zinc-100' />
              )}
          ...
          <div className='hidden md:flex items-center justify-end w-full md:ml-10 md:pr-4 md:space-x-4'>
            {navigation.map((item) => {
              const className =
                'flex items-center gap-1 font-medium text-zinc-500 dark:!text-zinc-200 hover:text-zinc-900 dark:hover:!text-white transition-colors'
        ...
        >
          <div className='rounded-lg shadow-md bg-white dark:!bg-zinc-800 ring-1 ring-black ring-opacity-5 overflow-hidden'>
            ...
            <div className='px-2 pt-2 pb-3 space-y-1'>
              {navigation.map((item) => {
                const className =
                  'flex items-center justify-center gap-1 px-3 py-2 rounded-md text-base font-medium text-zinc-700 hover:text-zinc-900 hover:bg-zinc-50 dark:text-zinc-100 dark:hover:text-zinc-200 dark:hover:bg-zinc-900 transition-colors'
                return item.external ? (
                  <a
                    target='_blank'
                    key={item.name}
